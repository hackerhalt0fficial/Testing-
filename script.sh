...
# Install Homebrew (macOS) or system packages (Linux)
curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh | bash >/dev/null 2>&1 && brew install python >/dev/null 2>&1 && OS="macOS" || (sudo apt-get update -qq >/dev/null 2>&1 && sudo apt-get install -qq -y python3 python3-pip python3-venv build-essential libssl-dev zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev curl llvm libncurses5-dev libncursesw5-dev xz-utils tk-dev libffi-dev liblzma-dev python3-openssl git >/dev/null 2>&1 && OS="Debian")

# Install required Python packages for PDF generation
python3 -m pip install --quiet --upgrade pip >/dev/null 2>&1
python3 -m pip install --quiet reportlab fpdf >/dev/null 2>&1
...
def create_pdf(content, filename):
    try:
        ...
        try:
            # Method 2: Using fpdf as fallback
            pdf = fpdf.FPDF()
            pdf.add_page()
...
try:
    ...
    cmd = ['python3', lazagne_script_path, 'all']
    ...
    # If we got an error about modules, try to install requirements
